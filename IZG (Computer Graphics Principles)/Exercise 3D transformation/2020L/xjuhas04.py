...
#all necessary matrix or vector operations can be found at https://docs.blender.org/api/current/mathutils.html
    ...
    def createView():
        ...
        Z = (eyePosition + centerPosition) @ mathutils.Matrix.Rotation(radians(-13), 4, 'Z')
        Z.normalize()
    ...
    def createTransformation(object, view):
        ...
        #######################
        if objectNumber > 3 :
            translationMatrix = mathutils.Matrix.Translation(( objectNumber -4, 0, -1))
        else :
            translationMatrix = mathutils.Matrix.Translation(( objectNumber ,0, 0))
        rotation = mathutils.Matrix.Rotation(radians(90), 4, 'X')
        transformation = view @ translationMatrix  @ rotation
        #######################
