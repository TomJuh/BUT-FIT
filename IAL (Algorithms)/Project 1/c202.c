...
void Stack_Init( Stack *stack ) {
    if (stack == NULL) //if stack isn't null set default index value
 ...
 * Vyvarujte se zejména konsrukce typu "if ( cond ) b=true else b=false".
 *t
...
int Stack_IsEmpty( const Stack *stack ) {
    if (stack->topIndex != -1) //if stack index is at default value it's empty
        return FALSE;
    else 
        return TRUE;
}
...
int Stack_IsFull( const Stack *stack ) {
    if (stack->topIndex == (STACK_SIZE-1)) //stack's range is from 0 to STACK_SIZE-1
        return TRUE;
    else 
        return FALSE;
}
...
void Stack_Top( const Stack *stack, char *dataPtr ) {
    ...
    *dataPtr = stack->array[stack->topIndex]; //assigns data to pointer from topIndex
}
...
void Stack_Pop( Stack *stack ) {
    if (!Stack_IsEmpty(stack)){
        stack->array[stack->topIndex] = '\0'; //set char at topindex to null
        stack->topIndex--; 
    }
}
...
void Stack_Push( Stack *stack, char data ) {
    if (Stack_IsFull(stack))
        ...
        stack->array[++stack->topIndex] = data; //if stack isn't full puts data on a higher index
	}
